...
private:
	...
	DataSet * createDataSet(const char *datasetName, Group *group, 
							enHDFTtypes hdfType, DataSpace *dataspace);		// Создает датасет
	DataSet * readDataSet(const char* name, Group *group);					// Читает датасет из группы
	void initDataSet();														// Создает пустой датасет
	void * addArrayToArray(void *firstArray, void *secondArray,
							int firstArraySize, int secondArraySize);		// Складывает 2 массива *void
public:
	const char * GetName();
	enHDFTtypes GetType();
	long GetLength();
	...
	Stream(const char * _name, enHDFTtypes _type,
			Group* _group, bool init);										// Флаг init ставится в единицу, если стрим нужно создать, а не открыть
	~Stream();
};
...
private:
	Group * OpenGroup(const char * groupName);							// Открывает группу в файле и возвращает хэндл на нее. Если такой группы нет, то создает
public:
